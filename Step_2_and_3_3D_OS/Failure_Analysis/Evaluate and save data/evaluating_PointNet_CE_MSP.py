...
def save_data_and_scores(id_loader, ood1_loader, ood2_loader, src_labels, tar1_labels, tar2_labels, src_MSP_scores, tar1_MSP_scores, tar2_MSP_scores, method, src, corruption):
    # save data and scores
    save_dir = f"results/{src}/{method}"
    ...
    #     np.save(os.path.join(save_dir, f"{src}_id_data_{i}.npy"), data.cpu().numpy())
    # for i, (data, label) in enumerate(ood1_loader):
    #     np.save(os.path.join(save_dir, f"{src}_ood1_data_{i}.npy"), data.cpu().numpy())
    # for i, (data, label) in enumerate(ood2_loader):
    #     np.save(os.path.join(save_dir, f"{src}_ood2_data_{i}.npy"), data.cpu().numpy())
...
def eval_ood_PointNet_CE_MSP(opt, config):
    ...
    save_data_and_scores(id_loader, ood1_loader, ood2_loader, src_labels, tar1_labels, tar2_labels, src_MSP_scores, tar1_MSP_scores, tar2_MSP_scores, "MSP", opt.src, opt.corruption)
    eval_ood_sncore(
...
def main():
    ...
    eval_ood_PointNet_CE_MSP(args, config)
